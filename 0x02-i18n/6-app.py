...
def get_locale():
    # Step 1: Check if 'locale' is provided as a URL parameter
    locale = request.args.get('locale')
    if locale and locale in app.config['LANGUAGES']:
        return locale

    # Step 2: Check the user's preferred locale if the user is logged in
    if g.user and g.user.get('locale') in app.config['LANGUAGES']:
        return g.user['locale']

    # Step 3: Use the best match from the 'Accept-Language' request headers
